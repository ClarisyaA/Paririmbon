    ...
    <div className="max-w-7xl mx-auto px-4 md:px-8 py-12 grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="glass-card p-6 rounded-2xl space-y-6 sticky top-28">
          <div>
            ...
            <div className="flex flex-wrap lg:flex-col gap-2">
              ...
              <button
                onClick={() => handleFilterClass("SundaKunoTerm")}
                className={`w-full text-left px-4 py-2.5 rounded-xl text-xs font-semibold uppercase tracking-wider transition ${
                  selectedClass === "SundaKunoTerm"
                    ? "gradient-gold-bg text-stone-950 font-bold shadow-lg shadow-amber-700/20"
                    : "text-stone-400 bg-stone-900/40 hover:bg-stone-900 hover:text-stone-200"
                }`}
              >
              ...
              <button
                onClick={() => handleFilterClass("Manuscript")}
                className={`w-full text-left px-4 py-2.5 rounded-xl text-xs font-semibold uppercase tracking-wider transition ${
                  selectedClass === "Manuscript"
                    ? "gradient-gold-bg text-stone-950 font-bold shadow-lg shadow-amber-700/20"
                    : "text-stone-400 bg-stone-900/40 hover:bg-stone-900 hover:text-stone-200"
                }`}
              >
      ...
      <section className="lg:col-span-3 space-y-8">
        ...
        <form onSubmit={handleSearchSubmit} className="glass-card p-2 rounded-2xl flex gap-2">
          ...
          <button
            type="submit"
            className="gradient-gold-bg hover:opacity-90 active:scale-95 text-stone-950 font-bold px-6 py-3 rounded-xl transition duration-200 text-sm whitespace-nowrap cursor-pointer shadow-lg shadow-amber-700/20"
          >
            Cari
          </button>
        </form>
        ...
        {queryParam && !loading && (
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-xs text-stone-400">
            <p>
              Ditemukan <span className="text-amber-400 font-bold">{total}</span> hasil pencarian untuk &quot;<span className="text-stone-200 italic font-medium">{queryParam === "*" ? "Semua" : queryParam}</span>&quot;
            </p>
        ...
        {loading ? (
          ...
          <>
            <div className="space-y-4">
              ...
              <div
                key={item.id}
                className="glass-card p-6 rounded-2xl flex flex-col md:flex-row justify-between gap-6 hover:border-amber-500/30 group transition duration-300"
              >
                <div className="space-y-3 flex-grow max-w-xl">
                  ...
                  {/* Label */}
                  <h3 className="text-2xl font-bold text-stone-100 font-display group-hover:text-amber-400 transition-colors">
                ...
                <div className="flex md:flex-col items-start md:items-end justify-between md:justify-between shrink-0 gap-4 pt-4 md:pt-0 border-t md:border-t-0 border-stone-800/60">
                  {item.scriptTerm && (
                    <div className="text-right">
                      <span className="text-stone-500 text-[10px] uppercase font-semibold tracking-wider block mb-1">Aksara Sunda</span>
                      <span className="font-sundanese text-xl text-amber-500 tracking-wider bg-stone-900/60 px-3 py-1.5 rounded-lg border border-stone-800/80 block select-none">
                        {item.scriptTerm}
                      </span>
                    </div>
                  )}
                  ...
                  <Link
                    ...
                    className="gradient-gold-bg text-stone-950 text-xs font-bold px-4.5 py-2.5 rounded-xl flex items-center gap-1.5 shadow-lg shadow-amber-800/10 hover:shadow-amber-500/20 transition-all select-none hover:scale-105 active:scale-95 cursor-pointer ml-auto md:ml-0"
                  >
                    <span>Jelajahi Relasi</span>
                    <span>→</span>
                  </Link>
                </div>
              </div>
          ...
          {selectedLimit !== "all" && totalPages > 1 && (
            <div className="flex items-center justify-between pt-6 border-t border-stone-900">
              <button
                ...
                type="button"
                className={`px-4 py-2 rounded-xl text-xs font-semibold uppercase tracking-wider transition flex items-center gap-1 cursor-pointer ${
                  currentPage === 1
              ...
              <div className="hidden sm:flex items-center gap-1.5">
                {Array.from({ length: totalPages }).map((_, idx) => {
                  const pageNum = idx + 1;
                  // Render a maximum of 7 page buttons, centered around current page
                  if (totalPages > 7) {
                    if (pageNum !== 1 && pageNum !== totalPages && Math.abs(currentPage - pageNum) > 2) {
                      if (pageNum === 2 || pageNum === totalPages - 1) {
                        return <span key={pageNum} className="text-stone-600 px-1 text-xs select-none">...</span>;
                      }
                      return null;
                    }
                  }
          ...
          <div className="glass-card p-12 rounded-2xl text-center space-y-4">
            <SearchX className="w-12 h-12 text-stone-500 mx-auto" />
            <h3 className="text-xl font-bold text-stone-200 font-display">Kosakata Tidak Ditemukan</h3>
            <p className="text-stone-400 text-sm max-w-sm mx-auto">
              Tidak ditemukan kosakata yang cocok dengan kata kunci &quot;<span className="text-stone-200">{queryParam === "*" ? "Semua" : queryParam}</span>&quot;. Coba cari kata kunci lainnya.
            </p>
          </div>
        ) : (
          <div className="glass-card p-12 rounded-2xl text-center space-y-4">
            <Compass className="w-12 h-12 text-amber-500/70 mx-auto animate-pulse" />
            <h3 className="text-xl font-bold text-stone-200 font-display">Mulai Penelusuran Semantik</h3>
            <p className="text-stone-400 text-sm max-w-sm mx-auto">
              Ketikkan kata kunci di atas untuk mencari istilah dalam naskah kuno Sunda. Contoh pencarian populer: <strong className="text-amber-500">sanghyang</strong>, <strong className="text-amber-500">tapa</strong>, atau <strong className="text-amber-500">raga</strong>.
            </p>
          </div>
        )}
      </section>
